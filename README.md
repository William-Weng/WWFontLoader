...
| `fontSize(for:in:maxLines:strategy:fontSize:fontFactor:) `| 只計算字級大小，不回傳文字框尺寸 |
| `textSize(for:font:constrainedTo:maxLines:strategy:) `| 計算文字在指定字型下的實際佔用尺寸 |
| `calculate(for:in:maxLines:strategy:fontSize:fontFactor:font:) `| 同時計算字級與文字框尺寸 |
